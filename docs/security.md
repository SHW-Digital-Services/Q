...
Q applies capability checks in addition to the existing staff and administrator roles. Staff permissions distinguish CRM, support, billing, security-administration, and aggregate-export operations. Partner administrators (`partner_admin`, shown as Admin) retain all capabilities and are exempt from application MFA requirements, including personal data export and account deletion. The exemption uses the authenticated user's database profile role. For staff, high-impact billing changes, security administration, credential recovery, role changes, and aggregate exports require an `aal2` Supabase session issued within the configured privileged-session window, in addition to the necessary permissions. Personal data export and account deletion continue to require recent MFA for non-admin users. MFA remains available to administrators as an optional account security feature.
