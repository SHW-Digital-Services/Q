...
Phases 0–3 provide canonical URLs, opaque errors, no-store responses, AAL2 and capability checks, atomic webhook claims, privacy receipts, retention rules, and server-side audit events. Phase 4 adds browser policy headers and a WebCrypto/IndexedDB storage foundation; the remaining synchronous localStorage consumers must be migrated before claiming all local records are encrypted. Phase 5 constrains trusted context and approved models while retaining a hosted-AI kill switch and includes a repeatable prompt-injection regression corpus (`npm run check:ai-safety`). Phase 6 adds dependency review, secret scanning, SBOM generation, and GitHub Actions pinned to reviewed release commits.
