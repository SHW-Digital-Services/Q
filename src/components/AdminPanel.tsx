...
export const AdminPanel: React.FC<AdminPanelProps> = ({ enabled, onToggle, onClose, onPreview }) => {
  ...
  const createTask = async (event: React.FormEvent) => {
    ...
    try {
      const payload = { ...taskForm, title: taskForm.title.trim(), startAt: taskForm.startAt ? new Date(taskForm.startAt).toISOString() : null, dueAt: taskForm.dueAt ? new Date(taskForm.dueAt).toISOString() : null, assignedTo: taskForm.assignedTo || null };
      const response = await fetch(selectedTask ? `/api/v1/admin/crm/users/${customer.identity.id}/tasks/${selectedTask.id}` : `/api/v1/admin/crm/users/${customer.identity.id}/tasks`, { method: selectedTask ? 'PATCH' : 'POST', headers: await getAuthHeaders(), body: JSON.stringify(payload) });
      await parseJsonResponse(response);
      await openCustomer(customer.identity.id);
      setTaskModalOpen(false);
      setSelectedTask(null);
      setTaskForm({ title: '', description: '', status: 'open', priority: 'normal', startAt: '', dueAt: '', assignedTo: '' });
      setCustomerMessage(selectedTask ? 'Task updated.' : 'Task created.');
