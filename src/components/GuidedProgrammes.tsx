...
export function GuidedProgrammes() {
  ...
  return <PremiumGate title="Guided programmes" description="Short, self-paced exercises for boundaries, connection and confidence. Pause or skip an exercise whenever you need.">
    ...
    {!programme ? <div className="grid gap-3 sm:grid-cols-3">{items.map(p=><button key={p.id} onClick={()=>{setSelected(p.id);setStep(0);}} className="rounded-xl border border-violet-200 bg-violet-50 p-4 text-left hover:bg-violet-100"><h3 className="font-bold">{p.title}</h3><p className="my-2 text-sm">{p.summary}</p><p className="text-xs font-semibold">{progress.find(x=>x.id===p.id)?.completed.length||0} of {p.sessions.length} sessions complete</p></button>)}</div> : <div>
      <button className="mb-3 text-sm font-bold text-violet-700 underline" onClick={()=>setSelected('')}>All programmes</button><h3 className="text-lg font-bold">{programme.title}</h3>
      <div className="my-3 flex flex-wrap gap-2">{programme.sessions.map((s,i)=><button key={s.title} aria-pressed={i===step} onClick={()=>setStep(i)} className={`rounded-lg border px-3 py-2 text-sm ${i===step?'bg-violet-700 text-white':'bg-white'}`}>{saved.completed.includes(i)?'✓ ':''}Session {i+1}</button>)}</div>
      <h4 className="font-bold">{programme.sessions[step].title}</h4><p className="my-3 text-sm leading-6">{programme.sessions[step].body}</p>
      <label className="block text-sm font-semibold">{programme.sessions[step].prompt}<textarea maxLength={5000} value={saved.notes[step]||''} onChange={e=>update({...saved,notes:{...saved.notes,[step]:e.target.value}})} className="mt-2 block min-h-28 w-full rounded-xl border border-slate-300 p-3 font-normal" placeholder="Optional reflection — saved on this device" /></label>
      ...
      <p className="mt-3 text-xs text-slate-500">Reflections stay on this device unless you enable programme continuity in Profile. These exercises are not therapy.</p>
    </div>}
  </PremiumGate>;
}
