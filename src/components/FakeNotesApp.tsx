...
export const FakeNotesApp: React.FC<Props> = ({ onUnlock, requiredPin }) => {
  ...
  const hiddenUnlock = () => { const now = Date.now(); clicks.current = [...clicks.current.filter(time => now - time < 1200), now]; if (clicks.current.length >= 4) { clicks.current = []; if (requiredPin) setShowUnlock(true); else onUnlock(); } };
