...
export function ContinuitySettings(){
  ...
  return <section className="my-5 rounded-2xl border border-violet-200 bg-white p-5"><h2 className="text-lg font-bold">Cross-device continuity <span className="text-xs text-violet-700">Premium</span></h2><p className="my-3 text-sm text-slate-600">Opt in on each device to keep selected data in your Q account. Selected content is stored in Q’s cloud; this is not end-to-end encryption. Device locks and passwords are never synced.</p>
    ...
    {conflict&&<div className="mt-4 rounded-xl bg-amber-50 p-4 text-sm"><p className="mb-3">No data has been replaced. Export both copies before choosing. Using the cloud copy also keeps a recovery copy on this device.</p><div className="flex flex-wrap gap-2"><button className="rounded border p-2" onClick={()=>downloadSnapshot(readSnapshot(userId,config.kinds))}>Export this device</button><button className="rounded border p-2" onClick={()=>run('push')}>Keep this device’s copy</button><button className="rounded border p-2" onClick={()=>run('pull')}>Use cloud copy</button></div></div>}
